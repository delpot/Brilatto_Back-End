...
class ModelController {
  ...
  public async getAllModelsByCategoryId(req: Request, res: Response) {
    return modelService
      .getModelsByCategoryId(req.params.categoryId)
      .then((models) => {
        return res.status(200).json(models);
      })
      .catch((error) => res.status(500).json(error));
  ...
  async createModel(req: Request, res: Response) {
    ...
    const createdModel = modelService.createJewelModel(
      categoryId,
      name,
      photo,
      description
  ...
  async updateModel(req: Request, res: Response) {
    return modelService
      .getModelByIdAndUpdate(req.params.id, req.body)
      .then((updatedModel) => {
  ...
  async softDeleteModel(req: Request, res: Response) {
    return modelService
      .getModelByIdAndSoftDelete(req.params.id)
  ...
  async hardDeleteModel(req: Request, res: Response) {
    return modelService
      .getModelByIdAndHardDelete(req.params.id)
      .then((deletedModel) => {
